    ...
    if isinstance(instance, ChangeloggableMixin):
        ...
        if changed:
            if created:
                ...
                fields = json.loads(json_dumps(fields))
                ChangeLog.add(instance, loggedIn.current_user, loggedIn.address, ACTION_CREATE, fields, id=last_saved['id'])
            else:
...
def journal_delete_handler(sender, instance, using, **kwargs):
    if isinstance(instance, ChangeloggableMixin):
        ...
        ChangeLog.add(instance, loggedIn.current_user, loggedIn.address, ACTION_DELETE, json.loads(json_dumps(instance.get_all_fields())), id=last_saved['id'])
