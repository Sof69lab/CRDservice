...
class ChangeLog(models.Model):
    ...
    def add(cls, instance, user, ipaddress, action_on_model, data, id=None):
        ...
        if log.model == "Замечания":
            log.record_id = str(instance.project_dogovor[4:8])+'-'+str(instance.num_reestr)+'/'+str(instance.num_remark)+'/'+str(instance.remark_v)
        if log.model == "Реестры":
            log.record_id = str(instance.project_dogovor[4:8]) + '-' + str(instance.num_reestr)
        if log.model == "Файлы":
